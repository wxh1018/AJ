function 山海经() {
    ...
    this.device = { x: device.width, y: device.height }
    let that = this
    // 初始化
    this.init = function () {
    ...
    this.合并 = function (col, row, area) {
        ...
        function merge(col, row, area) {
            ...
            let [xs, ys, xn, yn] = area || [180, 450, 900, 1900];
            col = col || 12;
            row = row || 6;
            tlog(xs, ys, xn, yn)
            let xlength = xn - xs
    ...
    function isimg(name) {
        let img = images.findImage(captureScreen(), images.read(that.src + name), { threshold: .8 })
        return img
    }
    function isfull(x, y) {
    ...
    function findImgmerge(col, row) {
        ...
        // x: 150-900   5列
        // 先纵向循环 再横向循环
        let [xs, ys, xn, yn] = [150, 1100, 900, 1900];
        col = col || 5;
        row = row || 12;
        tlog(xs, ys, xn, yn)
        let xlength = xn - xs
    ...
    function checkImg(img, point) {
        ...
        var arr = points.matches.map(v => {
            return { x: v.point.x, y: v.point.y }
        })
        log('符合条件的坐标', arr)
        if (arr.length < 1) {
            log('暂无')
